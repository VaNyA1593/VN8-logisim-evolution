...
for line in lines:
    ...
    if opcode in ["jmp", "cal"] and len(parts) == 2 or opcode in ["brh"] and len(parts) == 3:
        inst_counter += 2
    else:
        inst_counter += 1
...
for line in lines:
    ...
    # ---- LDI ----
        ...
        if parts[2] in defines:
            imm = defines[parts[2]]
        elif parts[2] in labels:
            imm = format(labels[parts[2]], "08b")
        else:
    ...
    # ---- JMP ----
        ...
        if len(parts) == 2:
            ...
            elif parts[1] in defines:
                addr = defines[parts[1]]
            else:
                addr = format(int(parts[1], 0), "016b")
    ...
    # ---- BRH ----
        ...
        if len(parts) == 3:
            ...
            bin_inst = f"0110{r2}{r1}01{cond_bits}"
            program.append(to_hex(bin_inst) + "\n")
